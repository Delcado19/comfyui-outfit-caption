...
class NvidiaNimOutfitCaption:
    ...
    DESCRIPTION = (
        "Captions a single outfit reference image with NVIDIA NIM-hosted vision models, describing only "
        "visible clothing, footwear and accessories for virtual try-on (VTON) prompts. Returns both the "
        f"raw QA caption and a ready-to-use VTON prompt. Needs an NVIDIA API key via the {DEFAULT_ENV} "
        ...
        return {
            "required": {
                ...
                "model": (
                    ...
                    {
                        "tooltip": (
                            "Vision model to call, ranked by tested rating (🟢/🟡/🔴/⚪) and confirmed against a "
                            "live NVIDIA probe. Refresh via the node's session-key panel. Ignored when custom_model is set."
                        )
                    },
                ),
                "custom_model": (
                ...
                "api_key_env": (
                    "STRING",
                    {
                        "default": DEFAULT_ENV,
                        "multiline": False,
                        "tooltip": "Environment variable holding your NVIDIA API key. Ignored while a session key is set on the node.",
                    },
                ),
                "timeout_seconds": (
                    "FLOAT",
                    {"default": 120.0, "min": 1.0, "max": 600.0, "step": 1.0, "tooltip": "Maximum time to wait for the NVIDIA API response, in seconds."},
                ),
                "max_image_mb": (
                    "FLOAT",
